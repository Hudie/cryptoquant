...
# unit of all following parameters: USD
LONG_GAP = [200, 250, 300, 350]
LONG_POSITION_SIZE_THRESHOLD = [10000 * i for i in [1, 2, 3, 4]]
SHORT_GAP = [-200, -250, -300, -350]
SHORT_POSITION_SIZE_THRESHOLD = [10000 * i for i in [1, 2, 3, 4]]
SIZE_PER_TRADE = 1000
...
class FutureArbitrage(ServiceBase):
    """ Service for cross future arbitrage. """
    ...
    async def process_msg(self):
        """ Function to process msg of both market data and transaction data. """
        try:
                    ...
                    if changes['instrument_name'] == FAR_FUTURE:
                        ...
                        if changes['orders']:
                            for order in changes['orders']:
                                if order['order_type'] == 'limit' and f_limit_order.if_placed and f_limit_order.label == order['label']:
                        ...
                        if changes['orders']:
                            for order in changes['orders']:
                                if order['order_type'] == 'limit' and p_limit_order.if_placed and p_limit_order.label == order['label']:
    ...
    async def balance_positions(self):
        """ Periodically balance positions when the positions of the trading pairs are not equal. """
        try:
