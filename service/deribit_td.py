...
"""
This service mainly deals with future related transaction data from Deribit API.
...
from common.account import CryptoTradingAccount
...
class DeribitTD(ServiceBase):
    """ Service of handlering Deribit transaction data. """
    ...
    async def pub_msg(self, account):
        """ Get transaction data from API websocket, then store it and pub it to zmq. """
        try:
            async with websockets.connect(DERIBIT_API_WEBSOCKET) as websocket:
                ...
                while websocket.open and self.state == ServiceState.started:
                    ...
                    if time.time() - lastheartbeat > 15:
                        raise websockets.exceptions.ConnectionClosedError(1003,
                                                                          'Serverside heartbeat stopped.')
    ...
    async def on_request(self):
        """
        Deal with user request from zmq. msg format is as follows:
